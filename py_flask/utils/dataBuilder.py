...
async def get_group_data():
    async with AsyncSession(db.engine) as db_ses:
        result = await db_ses.execute(
            select(StudentGroups)
            .options(
                joinedload(StudentGroups.users).joinedload(GroupUsers.user),
                joinedload(StudentGroups.scenario_groups).joinedload(ScenarioGroups.scenario)
            )
        )
...
async def get_user_data():
    async with AsyncSession(db.engine) as db_ses:
        result = await db_ses.execute(select(Users))
        ...
        for user in users:
            ...
            group_user = await db_ses.execute(select(GroupUsers).filter_by(user_id=user.id))
...
async def get_taAssignment_data():
    async with AsyncSession(db.engine) as db_ses:
        result = await db_ses.execute(select(TA_Assignments))
        ta_assignments = result.scalars().all()
