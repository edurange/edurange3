...
from py_flask.database.models import Users, StudentGroups
...
class LoginSchema(ma.SQLAlchemyAutoSchema):
    username = fields.String(required=True, validate=[Length(min=3, max=40)])
    password = fields.String(required=True, validate=[Length(min=3, max=40)])

    @validates_schema
    async def validate_login(self, data, **kwargs):
        username_input = data.get("username")
        password_plain_input = data.get("password")

        async with AsyncSessionLocal() as session:
            result = await session.execute(
                Users.__table__.select().where(Users.username == username_input)
            )
            user = result.scalar()
...
class RegistrationSchema(ma.SQLAlchemyAutoSchema):
    ...
    username = fields.String(required=True, validate=[
        Length(min=3, max=25, error="Username must be between 3 and 25 characters"),
        Regexp('^[a-zA-Z0-9]+$', error="Username must be alphanumeric")
    ])
    code = fields.String(required=True, validate=[Length(min=0, max=8)])
    password = fields.String(required=True, validate=[Length(min=6, max=40)])
    confirm_password = fields.String(required=True)

    @validates_schema
    async def validate_registration(self, data, **kwargs):
        username_input = data.get("username")
        ...
        async with AsyncSessionLocal() as session:
            ...
            group_result = await session.execute(
                StudentGroups.__table__.select().where(StudentGroups.code == code_input)
            )
            group = group_result.scalar()
            if group is None:
                custom_abort('Student group with this code not found.', 403)
...
class CreateGroupSchema(Schema):
    group_name = fields.String(required=True, validate=[
        Length(min=3, max=25, error="Group name must be between 3 and 25 characters"),
        Regexp('^[A-Za-z0-9]+(-?[A-Za-z0-9]+)*$', error="Group name must be alphanumeric")
    ])
    ...
    @validates_schema
    async def validate_groupCreate(self, data, **kwargs):
        group_name_input = data.get("group_name")
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                StudentGroups.__table__.select().where(StudentGroups.name == group_name_input)
            )
...
# Simplify other schemas similarly if needed
