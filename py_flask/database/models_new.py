...
class ChatMessages(Edu3Mixin, SurrogatePK, Model):
    ...
    user_id = reference_col("users",nullable=False) 
    user = relationship("Users", backref="group_users") # dev_fix (this is wrong)

    scenario_type = Column(db.String(50), nullable=False, unique=False)
    scenario_id = Column(db.Integer, nullable=False, unique=False) # re-added 
...
class BashHistory(Edu3Mixin, SurrogatePK, Model):
    ...
    user = relationship("Users", backref="responses") # added dev_fix (backref is wrong)
    ...
    scenario_id = Column(db.Integer, unique=True, nullable=False) # added  # may need to be reference column
    scenario = relationship("Scenarios", backref="responses", viewonly=True)  # dev_fix may not work
